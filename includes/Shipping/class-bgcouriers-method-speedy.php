...
class BGCouriers_Method_Speedy extends WC_Shipping_Method {
    ...
    public function calculate_shipping($package = []) {
        ...
        if (!BGCouriers_Settings::ships_to('speedy', $country, $courier)) { return; }
        ...
        try {
            ...
            BGCouriers_Logger::debug('no rate offered', ['courier' => 'speedy', 'country' => $country]);
            return;
        }
        $cost  = $quote->price;
        ...
        $included = $abroad ? true : BGCouriers_Settings::ship_in_total('speedy');
        $info     = 0.0;
        ...
        $free_now = !$abroad && WC()->cart && self::is_free((float) WC()->cart->get_subtotal(), BGCouriers_Settings::free_shipping('speedy', $method));
        if ($free_now) {
            $cost = 0.0;
        } elseif (!$included) {
