...
class BGC_Method_Speedy extends WC_Shipping_Method {
    ...
    public function calculate_shipping($package = []) {
        ...
        $sel     = BGC_Pricing::selection_for('speedy');
        $method  = $sel['method'];
        $site_id = $sel['site_id'];
        ...
        $included = BGC_Settings::ship_in_total('speedy');
        $info     = 0.0;
        if (!$included) {
            ...
            $cost = 0.0;
        } elseif (WC()->cart && self::is_free((float) WC()->cart->get_subtotal(), BGC_Settings::free_shipping('speedy', $method))) {
