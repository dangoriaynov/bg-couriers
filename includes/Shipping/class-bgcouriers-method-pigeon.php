...
class BGCouriers_Method_Pigeon extends WC_Shipping_Method {
    public function __construct($instance_id = 0) {
        $this->id = 'bgcouriers_pigeon';
    ...
    public function calculate_shipping($package = []) {
        ...
        if (!BGCouriers_Settings::courier_offerable('pigeon')) { return; }
        ...
        $courier = BGCouriers_Couriers::get('pigeon');
        ...
        if (!BGCouriers_Settings::ships_to('pigeon', $country, $courier)) { return; }
        ...
        $included = $abroad ? true : BGCouriers_Settings::ship_in_total('pigeon');
        $info     = 0.0;
        ...
        $free_now = !$abroad && WC()->cart && self::is_free((float) WC()->cart->get_subtotal(), BGCouriers_Settings::free_shipping('pigeon', $method));
        if ($free_now) {
            $cost = 0.0;
        } elseif (!$included) {
