...
class BGCouriers_Method_Evropat extends WC_Shipping_Method {
    public function __construct($instance_id = 0) {
        $this->id = 'bgcouriers_evropat';
        $this->instance_id = absint($instance_id);
        $this->method_title = __('Европът', 'bg-couriers');
        $this->method_description = __('Европът shipping (BG Couriers)', 'bg-couriers');
        $this->supports = ['shipping-zones', 'instance-settings'];
        $this->enabled = 'yes';
        $this->title = __('Европът', 'bg-couriers');
    ...
    public function calculate_shipping($package = []) {
        ...
        // "Enable Европът" was a switch that changed everything except the checkout. A zone entry is
        ...
        if (BGCouriers_Settings::courier_config('evropat') === null) { return; }
        ...
        $sel     = BGCouriers_Pricing::selection_for('evropat');
        $method  = $sel['method'];
        $site_id = $sel['site_id'];
        ...
        $courier = BGCouriers_Couriers::get('evropat');
        ...
        if (!BGCouriers_Settings::ships_to('evropat', $country, $courier)) { return; }
        ...
        try {
            ...
            BGCouriers_Logger::debug('no rate offered', ['courier' => 'evropat', 'country' => $country]);
            return;
        }
        ...
        $included = $abroad ? true : BGCouriers_Settings::ship_in_total('evropat');
        $info     = 0.0;
        ...
        $free_now = !$abroad && WC()->cart && self::is_free((float) WC()->cart->get_subtotal(), BGCouriers_Settings::free_shipping('evropat', $method));
        if ($free_now) {
            $cost = 0.0;
        } elseif (!$included) {
