...
class BGCouriers_Method_Expressone extends WC_Shipping_Method {
    public function __construct($instance_id = 0) {
        $this->id = 'bgcouriers_expressone';
        $this->instance_id = absint($instance_id);
        $this->method_title = __('Express One', 'bg-couriers');
        $this->method_description = __('Express One shipping (BG Couriers)', 'bg-couriers');
        $this->supports = ['shipping-zones', 'instance-settings'];
        $this->enabled = 'yes';
        $this->title = __('Express One', 'bg-couriers');
    ...
    public function calculate_shipping($package = []) {
        ...
        // "Enable Express One" was a switch that changed everything except the checkout. A zone entry is
        ...
        if (BGCouriers_Settings::courier_config('expressone') === null) { return; }
        ...
        $sel     = BGCouriers_Pricing::selection_for('expressone');
        $method  = $sel['method'];
        $site_id = $sel['site_id'];
        ...
        $courier = BGCouriers_Couriers::get('expressone');
        ...
        if (!BGCouriers_Settings::ships_to('expressone', $country, $courier)) { return; }
        ...
        try {
            ...
            BGCouriers_Logger::debug('no rate offered', ['courier' => 'expressone', 'country' => $country]);
            return;
        }
        $cost  = $quote->price;
        ...
        $included = $abroad ? true : BGCouriers_Settings::ship_in_total('expressone');
        $info     = 0.0;
        ...
        $free_now = !$abroad && WC()->cart && self::is_free((float) WC()->cart->get_subtotal(), BGCouriers_Settings::free_shipping('expressone', $method));
        if ($free_now) {
            $cost = 0.0;
        } elseif (!$included) {
