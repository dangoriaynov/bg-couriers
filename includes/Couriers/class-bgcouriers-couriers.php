...
final class BGCouriers_Couriers {
    ...
    public static function logo_file(string $id): string {
        $map = ['speedy' => 'speedy.png', 'econt' => 'econt.png', 'pigeon' => 'pigeon.png', 'boxnow' => 'boxnow.svg',
                'sameday' => 'sameday.png', 'expressone' => 'expressone.png', 'evropat' => 'evropat.png'];
        return $map[$id] ?? '';
    }
    ...
    public static function logo_url(string $id): string {
        $f = self::logo_file($id);
        if ($f === '' || !defined('BGCOURIERS_PATH') || !defined('BGCOURIERS_URL') || !is_readable(BGCOURIERS_PATH . 'assets/img/couriers/' . $f)) {
            return '';
        }
