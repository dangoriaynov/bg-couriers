 ...
 * Settings live as flat WC options (see feedback-settings-architecture):
 *   bgc_dual_currency
 *   bgc_speedy_{enabled,environment,username,password,client_id}
 *   bgc_speedy_{office|address|automat}_{enabled,price,currency,free_enabled,free_threshold}
 * The UI is rendered by BGC_WC_Settings (a WooCommerce Settings tab).
 */
class BGC_Settings {
    ...
    // ---- data accessors (used across the plugin) ----
    ...
    public static function dual_currency_enabled(): bool {
        return get_option('bgc_dual_currency', 'yes') === 'yes';
    }
    ...
    /** Per delivery-method config (default price/currency, free-shipping threshold). */
    public static function method_config(string $courier, string $method): array {
        ...
        return [
            ...
            'currency'       => get_option($p . 'currency', 'BGN'),
    ...
    /** Encrypt the password on save; keep the stored value when the field is left blank. */
    ...
    /** Custom WC settings field: Validate / Sync buttons (only when creds are present). */
