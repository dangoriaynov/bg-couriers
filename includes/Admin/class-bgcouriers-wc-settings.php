...
class BGCouriers_WC_Settings extends WC_Settings_Page {
    ...
    private function general_fields(): array {
        ...
        return [
            ...
            ['type' => 'select', 'id' => 'bgcouriers_picker_position', 'title' => __('Where to ask for the destination', 'bg-couriers'),
                'desc' => __('“With the shipping method” keeps the town/office/locker picker inside the order-review table, under the courier it belongs to. “Under the customer details” moves it up to just after the phone number - on a checkout with add-ons, gift options or a long totals table the picker is otherwise the very last thing a customer reaches, after everything optional. The courier choice and the price stay in the table either way. This is for the classic (shortcode) checkout: the block checkout renders in React and has no billing form of ours to sit under, so it keeps the picker with its rate whatever is chosen here.', 'bg-couriers'),
                'options' => ['rate' => __('With the shipping method (default)', 'bg-couriers'),
                              'details' => __('Under the customer details, after the phone', 'bg-couriers')],
                'default' => 'rate'],
            ...
            ['type' => 'checkbox', 'id' => 'bgcouriers_autolabel_wait',
                'title' => __('Wait for the dispatch day', 'bg-couriers'),
                'desc' => __('When the order names the day it ships (a delivery-date plugin such as Order Delivery Date stamps it on the order), issue the automatic waybill on the morning of that day instead of the moment the order reaches the status above. A waybill is a live shipment at the courier from the moment it exists: some couriers come for it the same day, and one issued weeks early sits at the courier for weeks. An order that names no day is labelled at once, and generating by hand is never held back.', 'bg-couriers'),
                'default' => 'yes'],
