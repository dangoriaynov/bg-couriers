    ...
    private function general_fields(): array {
        ...
        return [
            ...
            ['type' => 'checkbox', 'id' => 'bgcouriers_allmap_nearest', 'title' => __('“Nearest to you” on the map', 'bg-couriers'),
                'desc' => __('On the interactive map, offer to find the customer’s location and then show how far each pickup point is, which one is closest, and what collecting there saves against delivery to the address. The distances are worked out in the customer’s own browser and their position is never stored on the site. The one time the coordinates leave it is when the customer presses “find me” before naming a town - those are then reverse-geocoded to fill the town in for them (see the External services notice). On by default.', 'bg-couriers'),
                'default' => 'yes'],
