...
class BGCouriers_WC_Settings extends WC_Settings_Page {
    ...
    private function general_fields(): array {
        ...
        return [
            ...
            ['type' => 'select', 'id' => 'bgcouriers_delivery_position', 'title' => __('Where the delivery block goes', 'bg-couriers'),
                'desc' => __('The couriers, their prices and the fields of the one chosen move together as one block. “With the order review” leaves them where WooCommerce puts shipping rates, at the bottom of the page. “Under the customer details” moves the whole block up to just after the phone number: on a checkout with add-ons, gift options or a long totals table the couriers are otherwise the very last thing a customer reaches, after everything optional. This is for the classic (shortcode) checkout; the block checkout renders in React and keeps the rates where they are.', 'bg-couriers'),
                'options' => ['rate' => __('With the order review (default)', 'bg-couriers'),
