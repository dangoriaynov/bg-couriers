...
test('econt guest checkout to office, COD @econt', async ({ page }) => {
  ...
  console.log(`ECONT OFFICE Subtotal: ${sub}, Shipping: ${ship}, Total: ${total}`);
  expect(sub).toBeGreaterThan(0);
  expect(ship).toBeGreaterThan(0);
  expect(total).toBeGreaterThanOrEqual(sub + ship - 0.05);

  await fillGuestBilling(page, { first: 'Тест', last: 'Еконт', email: 'e2e-econt-office@example.com', phone: '0888123456' });
