...
5. Front-end: add product → checkout → choose Speedy → type postcode (e.g. 9300) → city auto-fills → pick office → price and order total update.
...
8. Checkout shows delivery prices in the shop currency only (no second currency anywhere); order totals + WooCommerce → Analytics unchanged.
