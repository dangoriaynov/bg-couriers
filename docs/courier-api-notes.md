...
- **Create / label / track:**
  ...
  - **Webhook auth (Webhook Guide v5, 2025-12; measured the hard way 2026-09-16 on a merchant's install):** one URL per partner, registered by BOX NOW support; authentication is a **request header** the partner configures in the profile (ours: `X-BGC-Webhook-Secret: <secret>`); a `datasignature` signing key exists but is given out only on request, and the guide does not say which bytes it signs or whether hex/Base64. Retries 5 min / 10 min / 1 h / 2 h / 24 h, then the hook may be disabled. Details in `docs/superpowers/specs/2026-07-08-boxnow-webhook.md`.
