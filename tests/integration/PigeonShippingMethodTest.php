...
final class PigeonShippingMethodTest extends WP_UnitTestCase {
    public function set_up() { parent::set_up(); BGCouriers_Schema::create(); BGCouriers_Rates::set('pigeon','office', BGCouriers_Zones::COUNTRY,4.99, get_woocommerce_currency());  bgcouriers_test_set_up_courier('pigeon');
        ...
        update_option('bgcouriers_pigeon_ship_in_total', 'yes');
    }
    ...
    public function test_calculate_shipping_uses_cached_rate_when_api_down(): void {
        ...
        BGCouriers_Couriers::register('pigeon', 'Pigeon Express', static function () use ($fake) { return $fake; });
        update_option('bgcouriers_pigeon_enabled', 'yes');
        WC()->session = WC()->session ?: new WC_Session_Handler();
        WC()->session->set('bgcouriers_method', 'office');

        $m = new BGCouriers_Method_Pigeon();
