...
final class EcontShippingMethodTest extends WP_UnitTestCase {
    public function set_up() { parent::set_up(); BGC_Schema::create(); BGC_Rates::set('econt','office',4.99,'BGN'); }
    ...
    public function test_calculate_shipping_uses_cached_rate_when_api_down(): void {
        ...
        BGC_Couriers::reset();
        BGC_Couriers::register('econt', 'Econt', static function () use ($fake) { return $fake; });
        update_option('bgc_econt_enabled', 'yes');
        WC()->session = WC()->session ?: new WC_Session_Handler();
        WC()->session->set('bgc_method', 'office');

        $m = new BGC_Method_Econt();
