...
/**
 * BGC_Tracking::classify()/stage() decide when the poller stops polling a waybill for good
 * (_bgc_track_done is never cleared) and, with auto-status on, when an order is completed - so a
...
final class TrackingStageTest extends TestCase {
    ...
    public function test_observed_speedy_operations(): void {
        $this->assertSame('transit', BGC_Tracking::classify('Получена информация за пратка'));
        $this->assertSame('cancelled', BGC_Tracking::classify('Анулиране'));
    }
    ...
    public function test_in_flight_delivery_phrasings_are_not_delivered(): void {
        foreach (['Предадена за доставка', 'Опит за доставка', 'Готова за доставка',
            ...
            $this->assertSame('transit', BGC_Tracking::classify($s), $s);
        }
    }
    ...
    public function test_real_delivery_still_classifies_as_delivered(): void {
        foreach (['Доставена пратка', 'Доставена', 'Доставено', 'Delivered'] as $s) {
            $this->assertSame('delivered', BGC_Tracking::classify($s), $s);
        }
    }

    public function test_returns_and_cancellations(): void {
        $this->assertSame('returned', BGC_Tracking::classify('Върната пратка'));
        $this->assertSame('cancelled', BGC_Tracking::classify('Отказана от получателя'));
    }
    ...
    public function test_stage_without_a_phase_reads_the_text(): void {
        $t = new BGC_Tracking('1', 'X', [['code' => 'X', 'name' => 'Доставена пратка', 'date' => '']]);
    ...
    public function test_phase_overrides_the_text(): void {
        ...
        $this->assertSame('transit', (new BGC_Tracking('1', '1', $ev, 'OUT_FOR_DELIVERY'))->stage());
        $this->assertSame('delivered', (new BGC_Tracking('1', '1', $ev, 'DELIVERED'))->stage());
        $this->assertSame('returned', (new BGC_Tracking('1', '1', $ev, 'RETURN_TO_SENDER'))->stage());
        $this->assertSame('returned', (new BGC_Tracking('1', '1', $ev, 'DELIVERED_BACK_TO_SENDER'))->stage());
        $this->assertSame('transit', (new BGC_Tracking('1', '1', $ev, 'IN_TRANSIT'))->stage());
    }
    ...
    public function test_cancellation_survives_a_non_terminal_phase(): void {
        ...
        $this->assertSame('cancelled', (new BGC_Tracking('1', '128', $ev, 'IN_TRANSIT'))->stage());
    }
}
