...
final class FreeShippingTest extends TestCase {
    ...
    public function test_is_free_at_or_above_threshold(): void {
        ...
        $this->assertTrue(BGCouriers_Method_Speedy::is_free(50.0, $cfg));   // exactly at threshold
        $this->assertTrue(BGCouriers_Method_Speedy::is_free(75.0, $cfg));   // above
    }
    public function test_is_free_below_threshold(): void {
        $this->assertFalse(BGCouriers_Method_Speedy::is_free(49.99, ['enabled' => true, 'threshold' => 50.0]));
    }
    public function test_is_free_disabled_or_zero_threshold(): void {
        $this->assertFalse(BGCouriers_Method_Speedy::is_free(100.0, ['enabled' => false, 'threshold' => 50.0]));
        $this->assertFalse(BGCouriers_Method_Speedy::is_free(100.0, ['enabled' => true, 'threshold' => 0.0]));
    }
    ...
    public function test_free_shipping_auto_enabled_by_positive_threshold(): void {
        ...
        Functions\when('get_option')->alias(function ($name, $default = false) {
            return ['bgcouriers_speedy_free_threshold' => '60'][$name] ?? $default;
        });
        $f = BGCouriers_Settings::free_shipping('speedy');
    ...
    public function test_courier_level_threshold_overrides_the_per_method_one(): void {
        Functions\when('get_option')->alias(function ($name, $default = false) {
            return ['bgcouriers_speedy_free_threshold' => '50', 'bgcouriers_speedy_office_free_threshold' => '30'][$name] ?? $default;
        });
        $this->assertSame(50.0, BGCouriers_Settings::free_shipping('speedy', 'office')['threshold']);
    }

    public function test_per_method_threshold_applies_only_when_courier_level_is_empty(): void {
        Functions\when('get_option')->alias(function ($name, $default = false) {
            return ['bgcouriers_speedy_office_free_threshold' => '30'][$name] ?? $default;
        });
        $f = BGCouriers_Settings::free_shipping('speedy', 'office');
        $this->assertTrue($f['enabled']);
        $this->assertSame(30.0, $f['threshold']);
        // A sibling method without its own threshold stays paid.
        $this->assertFalse(BGCouriers_Settings::free_shipping('speedy', 'address')['enabled']);
    }
}
