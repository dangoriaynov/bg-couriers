...
final class EncryptionTest extends TestCase {
    public function test_round_trip(): void {
        $key = str_repeat('k', 32);
        $cipher = BGCouriers_Encryption::encrypt('s3cret-pass', $key);
        $this->assertNotSame('s3cret-pass', $cipher);
        $this->assertSame('s3cret-pass', BGCouriers_Encryption::decrypt($cipher, $key));
    }
    public function test_empty_is_empty(): void {
        $this->assertSame('', BGCouriers_Encryption::encrypt('', str_repeat('k', 32)));
    }

    public function test_new_values_use_gcm_format(): void {
        $cipher = BGCouriers_Encryption::encrypt('hello', str_repeat('k', 32));
    ...
    public function test_legacy_cbc_still_decrypts(): void {
        $key    = str_repeat('k', 32);
        $aesKey = substr(hash('sha256', $key, true), 0, 32); // same derivation BGCouriers_Encryption uses
        $iv     = random_bytes(16);
        ...
        $this->assertSame('old-secret', BGCouriers_Encryption::decrypt($legacy, $key));
    }
    ...
    public function test_tampered_gcm_fails_closed(): void {
        ...
        $this->assertSame('', BGCouriers_Encryption::decrypt($tampered, $key));
    }
}
