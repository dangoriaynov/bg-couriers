 ...
 * these feed the checkout help box (BGCOURIERS.emergency) after repeated checkout failures.
 *
 * @group core
 */
final class EmergencyContactTest extends TestCase {
    ...
    public function test_emergency_returns_configured_phone_and_message(): void {
        Functions\when('get_option')->alias(function ($option, $default = '') {
            $map = ['bgcouriers_emergency_phone' => '+359 88 123 4567', 'bgcouriers_emergency_message' => 'Обадете ни се на телефона'];
            return $map[$option] ?? $default;
        });
        $e = BGCouriers_Settings::emergency();
